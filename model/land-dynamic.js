/*
    model/land-dynamic.js — Per-tick land simulation: fertility/water/pest/state transitions given the current environment snapshot.
*/

import { fb } from './utils.js';
import { timer } from './timer.js';

export class LandDynamicData {
    ...
    #newStateActive = {
        ...
        restoring: () => {
            ...
            return {
                ...
                life: Date.now() + timer.ingameToReal({ h: 12 }) * 1000,
                slot: {
                    cur: 0,
    ...
    water() {
        ...
        if (this.#curTemp > this.#averageTemp && this.#rainFall <= 0) {
            land.water.cur += land.water.cur * -(1 - this.#humidity);
            changed = true;
        }
